...
- `constructed_reference.py` — Build the treatment the generator will not draw, onto the style it will.
...
- `edit_path_probe.py` — Can an EDIT model break the strokes that a text-to-image model will not?
...
- `generate_candidate_references.py` — Generate candidate two-glyph references from a style DESCRIPTION, and score them.
...
- `narrow_descriptions.py` — Which descriptions give the user no real choice, and what should be said?
...
- `reference_to_atlas_transfer.py` — Does the atlas keep the style of the reference the user PICKED?
...
- `synthesise_transforms.py` — Applying a treatment to a REFERENCE-sized glyph, rather than an atlas cell.
- `synthesised_reference_probe.py` — If no model will INVENT a stencil, can ours TRANSFER one it is handed?
...
- `within_prompt_diversity.py` — Do N seeds of ONE description give a user a real choice, or the same font N times?
