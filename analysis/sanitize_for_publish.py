...
WHAT IT LOOKS FOR
  ...
  secrets         API-key shapes, tokens, private keys. `.env.example` holds
                  placeholders and is expected to pass.
...
SKIP_SUFFIX = (".png", ".jpg", ".jpeg", ".pt", ".safetensors", ".woff2",
               ".otf", ".ttf", ".ico", ".gz", ".zip")
...
def main(argv=None):
    ...
    if args.fix:
        ...
        return 0
    if args.check and (touched or secret_hits):
        print("\nFAIL: the tree is not clean for publication.")
        return 1
    if not touched and not secret_hits:
        print("clean.")
    return 0
