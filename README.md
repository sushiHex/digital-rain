...
- **The instruments were audited harder than the model.** A retrieval baseline beats every checkpoint on both style metrics; five training runs identical but for the seed land 0.16 apart on the headline metric; scored against each metric's own noise, the four effects that survive are all regressions. Every retraction stays in the record. ([the audit](#the-most-interesting-result-isnt-the-model))
...
**2. Training-run variance was never measured, and it is larger than every effect ever chased.** Five runs identical except `--seed` gave char_acc means of **0.4914 / 0.6070 / 0.5869 / 0.6031 / 0.6558**. The comparison tool — used for every A/B here — declares seven of the ten *identical-config* pairs a significant char_acc improvement (up to `r=0.87`), and on DINOv2 it fires on **eight of ten**. For scale, the flagship result below is `r=0.768`. ([three runs](research/2026-08-13-training-run-variance-measured-at-last.md), [five](research/2026-09-13-five-runs-and-the-noise-floor-moved.md))

**3. Scored on every metric rather than one each, four effects do clear 2 SE — and every one of them is a regression.** The resolvable ones all live on composite, LPIPS and R-ACC, the metrics nobody was watching, which are 6–20× more stable than the two that were. Stacking two "improvements" is the largest real effect in the project, at **−8.9 SE** on the composite, and it is a regression. On three runs it was six effects; two runs more and a 2.5× narrower interval on σ took the two nearest the line back below it, which is what "xSE ranks, it does not establish" means in practice. ([ledger](research/2026-08-13-the-claim-ledger-corrected.md), [re-scored](research/2026-09-13-five-runs-and-the-noise-floor-moved.md))

![Run-to-run noise against every claimed effect](viz/out/variance_vs_effects.png)

*Top: five runs whose configs differ only in `--seed`. Bottom: every claim ÷ that metric's own run-to-run SD, columns ordered by that SD. Ringed cells clear 2 SE — all four are regressions, and all four sit in the three left-hand columns. The two on the right, char_acc and DINOv2, are the metrics every headline in this project was quoted against. Regenerate with `python viz/variance_vs_effects.py`.*
...
>
> n=44 unique fonts, CI resampling fonts *and* seeds, analysis pre-registered before the 9B arm finished. **The gap is real and ~60% larger than it looked.** The composite, R-ACC and IDENTITY rows were re-scored at six seeds on 2026-09-12 with the same statistic: IDENTITY resolves for the 9B by a hair (+0.0056, CI [+0.0007, +0.0107]); R-ACC (+0.0084, CI [−0.0063, +0.0233]) and the composite (+0.0099, CI [−0.0033, +0.0227]) do not. [`research/2026-09-12-six-seeds-for-the-other-three-metrics.md`](research/2026-09-12-six-seeds-for-the-other-three-metrics.md) [`research/2026-08-08-multiseed-the-4b-9b-gap-is-real-and-bigger.md`](research/2026-08-08-multiseed-the-4b-9b-gap-is-real-and-bigger.md)

**Three of six metrics show no difference** (LPIPS, R-ACC, the composite). The deficit sits on the two style axes, plus an identity edge of 0.0056 that is resolved and negligible.
...
**The demo gates its input.** Before loading the transformer, `app.py` scores the two supplied reference glyphs for mutual style agreement and refuses a pair that disagrees, because the model copies whatever style it is given and would otherwise spend a minute producing a font that changes style part-way through a word. The rejection names the feature that differs — weight, slant, fill, or topology — so it reads as a re-roll instruction rather than a refusal. The threshold is **provisional** (cut on the spread of coherent references, not on references a person judged unacceptable), so the guard **fails open**: anything it cannot score is generated normally, and the check can be unticked. It has now met human judgement once: on 96 candidate references the owner labelled usable or not, the gate's distance predicts acceptance at AUC 0.84 (registered bar passed, p = 0.0001; at the cut precision 0.90, recall 0.85, specificity 0.70) — but every rejected reference came from one generator, and each seed index in these sets is one noise draw shared across all twelve descriptions; hold the seed fixed and the gate's signal is gone (AUC 0.36 over the within-seed pairs, p = 0.89). Two of the seven references it wrongly passes rank first and eighth of 96 on coherence: coherent *wrong* styles. The cut stays provisional until a sample with one seed per reference and the generator hidden. ([gate](research/2026-08-21-the-reference-gate.md), [calibration](research/2026-09-13-the-gate-clears-its-bar-and-the-seed-clears-it-higher.md))
...
**A relational treatment transfers too — and the controls say what carries it.** Stencil is local: every stroke carries the instruction. Monospace is a statement about the *set*, that all letters share a width, and two glyphs can only hint at it by agreeing. Equalising the `Kg` pair did nothing (ink widths 231 and 214 px already agree; CV 0.402 → 0.403). Equalising `Mi` (301 and 57 px) on the same checkpoint pulled the other ninety-two letters' widths together, CV **0.382 → 0.272**, most of the way from the real-proportional band to the real-monospace one. Three registered controls then unbundled the reference: condensing both glyphs with their ratio kept reproduces 26% of that fall, narrowing the `M` alone 61%, and **fattening the `i` alone 97%**. The model is not reading "these two agree"; it reads each glyph's departure from its normal width and applies it to the letters of that kind — which is exactly what a monospace designer draws, a squeezed `m` and a slab-serifed `i`. The registered replication then held on most of it and failed one clause: fattening the `i` to 1.5× / 2× / 3.1× moves the widths by +0.022 / +0.044 / +0.105 — a graded dose, not a switch — and under a second source face the prediction held in full, while under a second seed the sign, the rank order and the verdict reproduced but the registered "fattened `i` ≈ equalised" did not (0.64 of the fall, against 0.97 and 0.96). The `i`-alone share is quoted as 0.64–0.97 now. What stays constant across the three runs is the fall in width spread, 36–39% each time; what varies is the route — condensing on one run, pure added weight on another — and every run hands back a heavier face (bbox fill 0.51 / 0.65 / 0.67 against 0.45 / 0.51 / 0.40 plain), which is the cost a product would have to gate. ([note](research/2026-09-12-a-relational-treatment-transfers-when-the-pair-can-carry-it.md), [replication](research/2026-09-13-the-dose-is-graded-and-the-order-holds.md), figure [`viz/out/relational_widths.png`](viz/out/relational_widths.png))
...
- **No comparison of two training runs here is established — measured 2026-08-13, re-measured on five runs 2026-09-13.** Five runs differing only in `--seed` gave char_acc means of **0.4914 / 0.6070 / 0.5869 / 0.6031 / 0.6558** (SD 0.0603, 4 df). Training noise is metric-dependent by ~20x, and the two metrics this project chased hardest are the least stable: char_acc SD 0.0603, DINOv2 0.0368, against composite 0.0031. Against its own metric's noise, **no headline claim reaches 2 SE on the metric it was originally quoted against** — 4B→9B DINOv2 0.85 SE, the licence filter 1.1, 4B→9B char_acc 0.86, the levers ~0.7, the LR fix 0.3 (SE = σ√2 for two single runs). Scored on *every* metric instead, four effects clear 2 SE, and all four are regressions (rank 64 + oversampling on composite, LPIPS and R-ACC; the licence filter on composite). Two that cleared on three runs no longer do: the 4B→9B LPIPS regression (−1.9 SE) and the LR fix's (−0.9). [`research/2026-09-13-five-runs-and-the-noise-floor-moved.md`](research/2026-09-13-five-runs-and-the-noise-floor-moved.md)
...
3. **Calibrate the gate on human judgement.** Its thresholds are cut from the coherent references' own spread, not from references a person rejected. The first calibration set now exists — 96 references the owner judged, 23 rejected — and the registered bar passed on it; what it cannot yet do is cut a threshold, because the rejections sit in one generator and two of its seeds. A fresh, seed-blind sample is the remaining judgment task.
