...
CONTROLS_DIR = os.path.join(REPO, "eval_runs", "_relational_refs_Mi_controls")
...
def measure():
    ...
    # The third registration's control arms, summarised against the SAME
    # plain arm (regenerated in the controls run; it reproduces the Mi one).
    plain = os.path.join(CONTROLS_DIR, "atlas_plain.png")
    if os.path.isfile(plain):
        base = cells(plain, "Mi")
        out["Mi_controls"] = {}
        for arm in ("monospace",) + CONTROL_ARMS:
            atlas = os.path.join(CONTROLS_DIR, f"atlas_{arm}.png")
            if os.path.isfile(atlas):
                s = summarise("Mi", base, cells(atlas, "Mi"))
                del s["cells"]
                out["Mi_controls"][arm] = s
    return out
...
def main(argv=None):
    ...
    if record is None:
        ...
        record = {k: v for k, v in record.items() if k != "Mi_controls"}
        print(f"drawing from {RECORD}")
    else:
        payload = {"_comment": "Generated by viz/relational_widths.py from the plain and monospace "
                               "atlases of the two registered relational-transfer runs. Per-cell ink "
                               "widths over the 92 cells not drawn in the reference; the summary "
                               "numbers the 2026-09-12 note quotes.",
                   "moved_px": MOVED_PX, "group": GROUP, "pairs": record}
        with open(RECORD, "w", encoding="utf-8", newline="\n") as fh:
            json.dump(payload, fh, indent=1)
        print(f"wrote {RECORD}")
        for arm, s in record.get("Mi_controls", {}).items():
            print(f"Mi/{arm:<13} cv {s['cv_plain']:.3f} -> {s['cv_mono']:.3f}  mean {s['mean_plain']:.1f} -> "
                  f"{s['mean_mono']:.1f}  sd {s['sd_plain']:.1f} -> {s['sd_mono']:.1f}  "
                  f"ink {s['ink_pixels_change']:+.1%}  fill {s['fill_plain']:.3f} -> {s['fill_mono']:.3f}  "
                  f"affine {s['affine_fit']['intercept']:.1f} + {s['affine_fit']['slope']:.2f} x")
        for pair in [p for p in record if p != "Mi_controls"]:
            s = record[pair]
    ...
    draw({k: v for k, v in record.items() if k != "Mi_controls"})
    return 0
